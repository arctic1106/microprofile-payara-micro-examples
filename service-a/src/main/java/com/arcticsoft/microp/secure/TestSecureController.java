package com.arcticsoft.microp.secure;
...
public class TestSecureController 
{
	...
	{
		if (key == null) throw new WebApplicationException("Unable to read privateKey.pem", 500);
	...
	{
		JWTAuth provider = JWTAuth.create(null, new JWTAuthOptions().addPubSecKey(new PubSecKeyOptions().setAlgorithm("RS256").setPublicKey(key)));
	...
	// NOTE: Expected format is PKCS#8 (BEGIN PRIVATE KEY) NOT PKCS#1 (BEGIN RSA PRIVATE KEY). See gencerts.sh
	private static String readPemFile() 
	{
		StringBuilder sb = new StringBuilder(8192);
		try (BufferedReader is = new BufferedReader(
			new InputStreamReader(TestSecureController.class.getResourceAsStream("/privateKey.pem"), StandardCharsets.US_ASCII))) 
		{
			String line;
